<form class="bravo-theme-gotrip-login-form y-gap-20" method="POST" action="{{ route('login') }}">
    ...
    <div class="col-12">
        ...
        @if(is_enable_registration())
            <p class="mt-10">{{ __("Don't have an account yet?") }} <a href="{{ url('/register') }}" class="text-blue-1">{{ __('Sign up for free') }}</a></p>
        @endif
    </div>
    <div class="col-12">
    ...
    <div class="col-12">
        <button class="button py-20 -dark-1 bg-blue-1 text-white w-100" type="submit">
            {{ __('Sign In') }}
