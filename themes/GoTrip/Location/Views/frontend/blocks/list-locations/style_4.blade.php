                ...
                @if($rows)
                    @foreach($rows as $key => $row)
                        ...
                        <div data-anim-child="slide-left delay-{{$key + 3}}" class="swiper-slide">
                            ...
                            <a href="{{$row->getDetailUrl()}}" class="citiesCard -type-2 ">
                                <div class="citiesCard__image rounded-4 ratio ratio-3:4">
                                    <img class="img-ratio rounded-4 js-lazy" data-src="{{$row->getImageUrl()}}" src="#" alt="image">
                                </div>
                                    ...
                                    @foreach($service_type as $k => $type)
                                        @php $count = $row->getDisplayNumberServiceInLocation($type) @endphp
                                            @if(!empty($count))
                                                ...
                                                @else
                                                    <div class="text-14 text-light-1">{{$count}} {{__('travellers')}}</div>
                                                @endif
                                            @endif
                                        @endforeach
                                    @else
                                        @if(!empty($text_service = $row->getDisplayNumberServiceInLocation($service_type)))
                                            <div class="text-14 text-light-1">{{$text_service}} {{__('travellers')}}</div>
                                        @endif
                                    @endif
                                </div>
                            </a>
