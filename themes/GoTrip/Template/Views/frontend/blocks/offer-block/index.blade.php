@if(!empty($list_item))
    <div class="bravo-offer layout-pt-md layout-pb-md">
        <div class="container">
            <div class="row y-gap-20">
                @foreach($list_item as $key=>$item)
                    <div data-anim="slide-up @if($key) delay-1 @endif" class="col-md-6">

                        <div class="ctaCard -type-1 rounded-4 ">
                            <div class="ctaCard__image ratio ratio-63:55">
                                <img class="img-ratio js-lazy" src="#" data-src="{{ get_file_url($item['background_image'],'full') ?? "" }}" alt="image">
                            </div>

                            <div class="ctaCard__content py-70 px-70 lg:py-30 lg:px-30">


                                <h4 class="text-40 lg:text-26 text-white">{!! clean($item['title']) !!}</h4>

                                <div class="d-inline-block mt-30">
                                    <a href="{{$item['link_more']}}" class="button px-48 py-15 -blue-1 -min-180 bg-white text-dark-1">{{$item['link_title']}}</a>
                                </div>
                            </div>
                        </div>

                    </div>
                @endforeach
            </div>
        </div>
    </div>
@endif
