<?php
    ...
    class Flight extends Bookable
    {
        ...
        {
            // dd($request->all());
            // VD:15072023 - Add not request type to getting custom popup screen
            if (!$request->type) {
            ...
            // VD:15072023 - Add not request type to getting custom popup screen
            if (!empty($request->flight_seat) && !$request->type) {
                foreach ($request->flight_seat as $flight_seat) {
                    $total_guests += $flight_seat['number'] ?? 0;
                     $total += $flight_seat['number'] * $flight_seat['price'];
                }
            } 
            // VD:15072023 - Add flight seat to getting custom popup screen
            else {
                foreach ($request->flight_seat as $key => $value) {
                    $total_guests += $value;
                    $total += $key * $value; 
                }
                session()->put('total_passengers_count', $total_guests);
                session()->put('service_type', $request->input('service_type'));
            }
            ...
            // VD:13082023 - Update the flight timing after booking
            if ($request->type == 'customize') {
                $booking->start_date = $request->departure_time;
                $booking->end_date = $request->arrival_time;
            } else {
        ...
        public static function search(Request $request)
        {
            ...
            */
            ...
            // dd($request->all());
            // $depart = $request->start;
            // $depart_date = date('Y-m-d', strtotime($depart));
            // $query =  DB::table('bravo_flight')
            // ->where('airport_from', $request->from_where)
            // ->where('airport_to', $request->to_where)
            // ->whereDate('departure_time', $depart_date)
            // ->where('status', 'publish')
            // ->paginate(10);
            // return $query;

            /*
            VD:13082023 - Set query based on flight page like search and default
            Get session from flightcontroller
            */
            if (session()->get('flight_page') == 'search') {

                $departTime = $request->start; //m-d-Y
                $depart_date = date('Y-m-d', strtotime($departTime));

                $query =  DB::table('bravo_flight')
                ->select('bravo_flight.*', 'flight_timings.departure_time', 'flight_timings.arrival_time')
                ->join('flight_timings', 'flight_timings.flight_id', '=', 'bravo_flight.id')
                ->where('bravo_flight.airport_from', $request->from_airport)
                ->where('bravo_flight.airport_to', $request->to_airport)
                ->where('bravo_flight.status', 'publish')
                ->whereDate('flight_timings.departure_time', $depart_date)
                ->paginate(10);
                return $query;
