            ...
            <div class="col-left">
                <form method="get" action="" class="filter-form filter-form-right d-flex justify-content-end">
                    ...
                    @if(!empty($booking_manage_others))
                        ...
                        ?>
                    @endif
                    <input type="text" name="s" value="{{ Request()->s }}" placeholder="{{__('Search by name or Booking ID')}}" style="max-width:46%" class="form-control">
        ...
        <div class="panel booking-history-manager">
            ...
            <div class="panel-body">
                <form action="" class="bravo-form-item">
                    <table class="table table-hover bravo-list-item">
                        ...
                        @foreach($rows as $row)
                            ...
                            <tr>
                                ...
                                <td>
                                    <ul>
                                        <li>{{__("Booking ID:")}} {{$booking->reference_id}} </li>
                                ...
                                <td>
                                    <span class="label label-{{$row->status}}" <?php if(isset($row->status) && $row->status == 'cancelled' || $row->status == 'Cancellation Request'){  ?> style="color: red;" <?php } ?> >{{$row->statusName}}  
                                    @if(isset($row->status) && $row->status == 'cancelled' || $row->status == 'Cancellation Request')
                                        <a class="btn btn-primary btn-info-booking" style="margin-top: 4px;" data-toggle="modal" data-target="#modal-booking-cancel-{{$booking->id}}">
                                                {{__("View Reason")}}
                                            </a>
                                    ...
                                    @if($service = $row->service)
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item" href="#modal_booking_detail" data-ajax="{{route('booking.modal',['booking'=>$booking])}}" data-toggle="modal" data-id="{{$booking->reference_id}}" data-target="#modal_booking_detail">{{__('Detail')}}</a>
                                        ...
                                        <br>
                                        <div>
                                            <a class="btn btn-primary" href="{{route('report.admin.booking.reply',['booking'=>$row])}}">{{__("Reply")}}</a>
                                        </div>
                                        <a href="#" class="btn btn-xs btn-primary btn-info-booking btn-open-modal" style="margin-top: 4px;" 
                                        data-booking-id="{{$booking->id}}" 
                                        data-reference-id="{{$booking->reference_id}}" 
                                        data-confirmation-number="{{$booking->confirmation_number}}">
                                                Add Number
                                        </a>
                   ...
                   <!-- Add Confirmation Number popup -->
                   <div class="modal fade" id="modal-confirmation-number">
                            <div class="modal-dialog modal-dialog-centered modal-small">
                                <div class="modal-content">
                                    <!-- Modal Header -->
                                    <div class="modal-header">
                                        <h4 class="modal-title">{{_('Add Number')}}</h4>
                                    </div>
                                    <!-- Modal body -->
                                    <div class="modal-body">
                                    <form action="{{route('report.admin.booking.addConfirmationNumber')}}" method="post">
                                        @csrf
                                        <div class="form-group">
                                            <input type="hidden" value="" name="booking_id" id="booking_id" />
                                            <input type="text" value="" class="form-control" name="confirmation_number" id="confirmation_number" />
                                            </br>
                                            <span class="btn btn-secondary pull-right" data-dismiss="modal">{{__("Close")}}</span>
                                            <button type="submit" class="btn btn-xs btn-primary btn-flat show_confirm pull-right" style="margin-right: 5px;">Add</button> 
                                        </div>
                                    </form>
                                    </div>
                                </div>
...
@push('js')
    <script>
        ...
        var confirmationNumber = $(this).data('confirmation-number');

        // Populate the modal fields with the corresponding data
        $('#booking_id').val(bookingId);
        $('#modal-confirmation-number-input').val(confirmationNumber);

        // Show the modal (optional if not already shown by data-toggle)
        $('#modal-confirmation-number').modal('show');
    });
    </script>
@endpush
