...
Route::group(['prefix'=>'user','middleware' => ['auth','verified']],function(){
    ...
    Route::post('/cancellation','BookingController@cancellationRquest')->name('user.booking.cancellation');
...
Route::group(['prefix'=>'user/enquiry-report'],function(){
    Route::get('/','EnquiryController@enquiryReport')->name("user.enquiry_report");
    Route::get('/bulkEdit/{id}','EnquiryController@enquiryReportBulkEdit')->name("user.enquiry_report.bulk_edit")->middleware(['signed']);
    Route::get('/{enquiry}/reply','EnquiryController@reply')->name('user.enquiry_report.reply');
    Route::post('/{enquiry}/reply/store','EnquiryController@replyStore')->name('user.enquiry_report.replyStore');
});


Route::group(['prefix'=>'user/booking-history'],function(){
    Route::get('{booking}/reply','BookingController@reply')->name('user.booking.reply');
    Route::post('{booking}/reply/store','BookingController@replyStore')->name('user.booking.replyStore');
});