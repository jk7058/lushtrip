@php
    $style = $style ?? '';
@endphp
<form action="{{ route("tour.search") }}" class="@if($style != 'carousel_v2')form bravo_form mainSearch -w-900 bg-white px-10 py-10 lg:px-20 lg:pt-5 lg:pb-20 rounded-100 @endif" method="get">
    @php $space_search_fields = setting_item_array('tour_search_fields');
        ...
        @if(!empty($space_search_fields))
            @foreach($space_search_fields as $field)
                ...
                @switch($field['field'])
                    ...
                    @break
                    @case ('guests')
                    @include('Tour::frontend.layouts.search.fields.guests')
                    @break
                @endswitch
            @endforeach
        @endif
