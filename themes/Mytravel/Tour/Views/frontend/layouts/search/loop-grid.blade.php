        ...
        <div class="position-absolute bottom-0 left-0 right-0 text-content">
            <div class="px-3 pb-2">
                @if(!empty($row->category_tour->name))
                    ...
                    <!-- <span class="text-gray-10 font-weight-normal font-size-14">{{$cat->name ?? ''}}  {{count($row->itinerary) - 1}} N / {{count($row->itinerary)}} D</span> -->
                     <span class="text-gray-10 font-weight-normal font-size-14">{{count($row->itinerary) - 1}}N / {{count($row->itinerary)}}D</span>
                @endif
