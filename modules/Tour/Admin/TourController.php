...
class TourController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($tour_name = $request->input('s'))) {
            $query->where('title', 'LIKE', '%' . $tour_name . '%')
                ->orwhere('reference_code', $tour_name)
                ->orwhere('reference_code', "#" . $tour_name);
    ...
    public function store(Request $request, $id)
    {
        ...
        if ($id < 0) {
            $row->reference_code = "#REF" . Str::random(9);
        } else {
            $row->reference_code = $request->reference_code;
        }
        // dd($row->reference_code); 
        ...
        $row->remark = $request->input('remark');
        $row->desclaimer = $request->input('desclaimer');
        $row->star_rate = $request->input('star_rate');
        // dd($row);
